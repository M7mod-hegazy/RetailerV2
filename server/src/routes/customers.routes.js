  ...
  const info = getDb()
    .prepare(
      `INSERT INTO customers
       (name, phone, code, opening_balance, credit_limit, is_blacklisted, blacklist_reason, is_active)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    )
    .run(
      payload.name,
...
router.put("/:id", (req, res) => {
  ...
  getDb()
    .prepare(
      `UPDATE customers
       SET name = ?, phone = ?, code = ?, opening_balance = ?, credit_limit = ?, is_blacklisted = ?, blacklist_reason = ?, is_active = ?
       WHERE id = ?`,
    )
    .run(
      payload.name,
...
router.delete("/:id", (req, res) => {
  try {
    ...
    if (err.message?.includes("FOREIGN KEY")) return res.status(409).json({ success: false, message: "لا يمكن حذف العميل لأنه مرتبط بفواتير أو مدفوعات" });
