...
function adjustStock({ item_id, warehouse_id, quantityDelta, movement_type, reference_type = null, reference_id = null, notes = null }) {
  const db = getDb();
  const tx = db.transaction(() => {
    ...
    ).run(item_id, warehouse_id, movement_type, quantityDelta, beforeQty, afterQty, reference_type, reference_id, notes);
  });
  tx();
}
