    ...
    <span>
      {parts.map((part, i) => 
        part.toLowerCase() === query.toLowerCase() 
          ? <mark key={i} className="bg-amber-500/20 text-amber-900 px-0.5 rounded-sm">{part}</mark> 
          : part
      )}
    </span>
  );
    ...
    <div className="relative" ref={containerRef}>
      <button 
        ...
        className={`flex items-center gap-2 h-10 px-3 rounded-xl border transition-all outline-none ${
          open 
            ? "bg-white border-amber-300 ring-4 ring-amber-500/10 shadow-sm" 
            ...
            <div className="max-h-[250px] overflow-y-auto no-scrollbar flex flex-col gap-0.5">
              <button
                ...
                className={`w-full text-right px-3 py-2 rounded-xl text-[12px] font-bold transition-colors ${
                  value === "" ? "bg-amber-50 text-amber-700" : "text-slate-600 hover:bg-slate-50"
                }`}
              >
                {placeholder}
              </button>
              {options.map((opt) => (
                <button
                  ...
                  className={`w-full text-right px-3 py-2 rounded-xl text-[12px] font-bold transition-colors ${
                    value === opt.value ? "bg-amber-50 text-amber-700" : "text-slate-600 hover:bg-slate-50"
                  }`}
                >
                  {opt.label}
                </button>
    ...
    <div className="flex flex-col sm:flex-row items-center gap-2 bg-slate-50/80 p-1.5 rounded-2xl border border-slate-100">
      <div className="flex items-center gap-1 bg-slate-200/50 p-1 rounded-xl shrink-0">
        <button 
          ...
          className={`px-3 py-1.5 rounded-lg text-[11px] font-black transition-all ${mode === "today" ? "bg-white text-amber-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
        >
          اليوم
        </button>
        <button 
          onClick={() => handleModeChange("single")}
          className={`px-3 py-1.5 rounded-lg text-[11px] font-black transition-all ${mode === "single" ? "bg-white text-amber-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
        >
          يوم محدد
        </button>
        <button 
          onClick={() => handleModeChange("range")}
          className={`px-3 py-1.5 rounded-lg text-[11px] font-black transition-all ${mode === "range" ? "bg-white text-amber-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
        >
          فترة
        </button>
      </div>
      ...
      >
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-white">
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center h-8 w-8 rounded-xl bg-amber-100 text-amber-600">
              <Command className="h-4 w-4" />
            </div>
            <span className="text-[13px] font-black text-slate-800 tracking-tight">
              {initialData?.id ? 'تعديل السحب' : 'تسجيل سحب جديد'}
            </span>
          </div>
        ...
        <div className="p-8 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">تصنيف السحب <span className="text-amber-500">*</span></label>
              <select 
                value={form.category_id} onChange={e => setForm({...form, category_id: e.target.value})}
                className="w-full h-12 bg-slate-50 border border-slate-200 rounded-xl px-4 text-[13px] font-bold outline-none focus:bg-white focus:border-amber-400 focus:ring-4 focus:ring-amber-500/10 appearance-none"
              >
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">البيان / المسحوب له</label>
            <input 
              placeholder="اكتب وصفاً أو اسم المستلم..." value={form.note} onChange={e => setForm({...form, note: e.target.value})}
              className="w-full h-12 bg-slate-50 border border-slate-200 rounded-xl px-4 text-[13px] font-bold outline-none focus:bg-white focus:border-amber-400 focus:ring-4 focus:ring-amber-500/10"
            />
          </div>
        </div>
...
// ----------------------------------------------------------------------
// Background Spline Animation - Amber
// ----------------------------------------------------------------------
const SplineHeader = () => (
...
export default function WithdrawalsListPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [wR, cR] = await Promise.all([
        api.get(`/api/withdrawals?date_from=${dateFrom}&date_to=${dateTo}${catFilter ? `&category_id=${catFilter}` : ""}`),
        api.get("/api/withdrawals/categories"),
      ]);
      setRows(wR.data.data || []);
  ...
  const filtered = useMemo(() => fuzzyFilterRows(rows, query, ["note", "category_name", "doc_no"]), [rows, query]);
  ...
  async function handleSave(formData) {
    ...
    try {
      if (formData.id) {
        await api.put(`/api/withdrawals/${formData.id}`, formData);
        toast.success("تم تعديل السحب");
      } else {
  ...
  function openCreate() {
    if (categories.length === 0) {
      toast.error("يجب إنشاء قسم مسحوبات أولاً");
      return;
    }
    setCmdData(null);
    setCmdOpen(true);
      ...
      <div className="relative z-10 px-8 pt-12 pb-8 max-w-5xl mx-auto w-full">
        <div className="flex flex-col items-center text-center gap-4 mb-10">
          ...
          <motion.p initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="text-[13px] font-medium text-slate-500 max-w-xl mx-auto leading-relaxed">
            دفتر تشغيلي لإدارة وتسجيل كافة المسحوبات والعهد. استخدم الفلاتر للبحث وتحديد فترات زمنية، واضغط على <kbd className="inline-flex items-center justify-center px-1.5 py-0.5 rounded border border-slate-200 bg-white text-[10px] font-mono text-slate-500 mx-1 shadow-sm">Ctrl+K</kbd> في أي وقت لتسجيل سحب جديد.
          </motion.p>
        </div>
            ...
            <div className="px-3 pt-1 pb-2 flex items-center justify-between border-b border-slate-100/50">
              <div className="flex flex-wrap items-center gap-4">
                <div className="flex items-center gap-1.5 cursor-default" title="إجمالي مسحوبات اليوم فقط">
                  ...
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">سحب اليوم:</span>
                  <span className="text-[12px] font-black font-mono text-amber-600">{fmt(stats.todayAmt)} ج.م</span>
                </div>
                      ...
                      <div className="flex flex-col gap-1.5">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="text-[15px] font-black text-zinc-900 leading-none">
                            <Highlight text={row.note || "سحب عام"} query={query} />
                          </span>
                        ...
                        <div className="flex items-center gap-3 text-[11px] font-bold text-slate-400 font-mono">
                          <span>{row.doc_no || `WD-${String(row.id).padStart(5, "0")}`}</span>
                        </div>
                      </div>
                    </div>
