...
export default function PurchaseReturnDetailPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const r = await api.get(`/api/purchases/returns/${id}`);
      const data = r.data.data;
      setDoc(data);
      await buildTimeline(data);
    } catch {
      toast.error("فشل تحميل مرتجع الشراء");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function buildTimeline(d) {
    ...
    try {
      ...
      while (cur.amendment_of) {
        const r = await api.get(`/api/purchases/returns/${cur.amendment_of}`);
        chain.unshift(r.data.data);
        cur = r.data.data;
      }
      cur = d;
      while (cur.amended_by) {
        const r = await api.get(`/api/purchases/returns/${cur.amended_by}`);
  ...
  async function handleCancel(reason) {
    try {
      await api.post(`/api/purchases/returns/${id}/cancel`, { reason });
