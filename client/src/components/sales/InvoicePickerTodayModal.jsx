...
export default function InvoicePickerTodayModal({ open, onClose, onSelectInvoice, customers: propCustomers }) {
  ...
  function handleRowClick(r) {
    if (itemSearch.trim()) {
      if (r.invoice_id) { setDetailItem({ id: r.invoice_id, invoice_no: r.invoice_no || r.doc_no, customer_name: r.customer_name, total: Number(r.unit_price) * Number(r.quantity), created_at: r.created_at }); setDetailOpen(true); }
    } else {
  ...
  const itemColumns = [
    ...
    { id: "invoice_no", header: "الفاتورة", width: 130, cellClass: "px-3 font-mono text-[11px] font-black text-slate-700", render: (r) => r.invoice_no || r.doc_no || "—" },
      ...
      <Modal open={open} onClose={onClose} title="اختيار فاتورة للمرتجع" maxWidth="max-w-5xl">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-4 rounded-sm bg-emerald-800 px-4 py-3">
            ...
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-emerald-300 uppercase tracking-widest">إجمالي المبيعات</span>
